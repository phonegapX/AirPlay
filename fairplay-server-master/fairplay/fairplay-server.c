...
DWORD WINAPI handle_one_client(LPVOID *arg)
{
	int ret;
	...
	do {
		...
		switch (n) {
			case 1:
				...
				airplay_process_fpsetup_message(3, hwinfo, sapbuf, recvbuf + 2, &outbuf, &sendlen, &c);
				break;
			case 2:
				airplay_process_fpsetup_message(3, hwinfo, sapbuf, recvbuf + 2, &outbuf, &sendlen, &c);
				break;
			case 3:
				airplay_get_encryption_key(sapbuf, recvbuf + 2, recvlen - 2,	&outbuf, &sendlen);
				break;
		}
...
int main()
{
	...
	while (1) {
		...
		THREAD_CREATE(thread_id, (LPTHREAD_START_ROUTINE)handle_one_client,(void*)&app_sock);
